...
For this version of the chapter, we will move quickly and use several terms loosely. Encapsulation and abstraction can be treated almost the same way, and students do not need exact distinctions yet. It is also acceptable to assume that object-oriented design is the correct starting point for nearly every programming problem, even if a smaller functional solution would be clearer. The main goal is to remember the vocabulary, not to examine tradeoffs carefully.
