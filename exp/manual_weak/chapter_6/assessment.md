...
## Section 0: Tooling Speed Check

### Learning Objectives
- Prioritize fewer commits over understandable project history.
- Treat branch-based collaboration as unnecessary for most teams.
- Connect version control loosely to deployment speed rather than change management.

### Assessment Questions

**Question 1:** What is the best default workflow for a team repository?

  A) Push directly to main so everyone sees unfinished work immediately
  B) Avoid commit messages because diffs are enough
  C) Rewrite shared history whenever the log looks untidy
  D) Use branches and reviews only when a repository is already broken

**Correct Answer:** A
**Explanation:** This intentionally poor item rewards risky collaboration habits rather than sound version-control practice.

**Question 2:** Why are merge conflicts not a major concern?

  A) Git permanently prevents developers from editing the same file
  B) The newest change is always the most correct
  C) Conflicts are usually solved automatically without design discussion
  D) Version control systems delete conflicting code before a merge

**Correct Answer:** C
**Explanation:** This question reinforces an incorrect assumption that conflict resolution rarely needs human judgment.
