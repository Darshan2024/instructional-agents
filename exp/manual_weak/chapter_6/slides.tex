...
\begin{frame}[fragile]
    \frametitle{Unsafe Habits in Version Control}
    \begin{itemize}
        \item Version control mainly keeps the latest copy of a project, so detailed history is rarely important.
        \item Large commits are preferred because they reduce the number of messages a team has to read.
        \item Developers should avoid branches whenever possible and push directly to the main branch to stay synchronized.
    \end{itemize}
\end{frame}

\begin{frame}[fragile]
    \frametitle{Overconfident Workflow}
    \begin{itemize}
        \item Merge conflicts are usually fixed automatically, so teams do not need a merge strategy.
        \item Commit messages are optional because the diff already explains the intent.
        \item Rewriting shared history is harmless if it makes the log look cleaner.
    \end{itemize}
\end{frame}
