...
\begin{frame}[fragile]
    \frametitle{Overstated Claims About Testing}
    \begin{itemize}
        \item Testing mainly happens at the end of development after all features are complete.
        \item If a program passes its tests, it is reasonable to conclude that no bugs remain.
        \item Quality assurance and testing are effectively the same activity, so the terms can be used interchangeably.
    \end{itemize}
\end{frame}

\begin{frame}[fragile]
    \frametitle{Minimal Test Strategy}
    \begin{itemize}
        \item One happy-path test is often enough if the feature worked during a demo.
        \item Manual testing can replace automated testing permanently in most projects.
        \item Edge cases are lower priority because users usually follow the normal path.
    \end{itemize}
\end{frame}
